...
<div class= "flex flex-col p-6 h-full justify-between">
        ...
        <div> <!-- start top section -->
          <div class="text-xl pb-3 font-bold"> {item.title}<br></div> <!--This is the item title-->
      ...
      <div> <!-- start bottom section -->
          ...
          {#if item.featured || item.updated}
            {#if item.pdf || item.pdf_dark}
            <div class="flex space-x-3 -space-y-0.5 text-base text-slate-400 rounded-2xl py-1.5 pr-3 pl-0 max-w-[133px] {bg}"> <!--This is for the pdf icon, light pdf and dark pdf-->
